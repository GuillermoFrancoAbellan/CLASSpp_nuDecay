...
int BackgroundModule::background_init() {
  ...
  if (pba->background_verbose > 0) {
    ...
    if ((pba->N_ncdm > 0) || (pba->Omega0_idr != 0.))  {
      ...
      if (pba->N_ncdm > 0){
        Neff += ncdm_->GetNeff(0.,pba->has_ncdm_decay_dr_ncdm, pba->has_negative_nu_mass); //GFA
        ncdm_->PrintNeffInfo(pba->has_ncdm_decay_dr_ncdm, pba->has_negative_nu_mass); //GFA
      }
      ...
      printf(" -> Total neutrino mass is M_tot = %g eV \n",pba->Mnu_tot); // GFA
  ...
  if ((pba->background_verbose > 0) && (pba->has_ncdm == _TRUE_)) {
    ncdm_->PrintMassInfo(pba->has_ncdm_decay_dr_ncdm);
  }
...
int BackgroundModule::background_initial_conditions(double* pvecback, /* vector with argument pvecback[index_bg] (must be already allocated, normal format is sufficient) */
  ...
  */
  ...
  if (pba->has_ncdm_decay_dr == _TRUE_) {
    ...
    for (const auto& [ncdm_id, dncdm_properties] : ncdm_->decay_dr_map_) {
      if (pba->has_ncdm_decay_dr_ncdm == _TRUE_) { // GFA
        if (dncdm_properties.dncdm_id == 0) {
    ...
    for (const auto& [ncdm_id, dncdm_properties] : ncdm_->decay_dr_map_) {
      ...
      // For the decay DNCDM --> NCDM + DR, one might want to take the limit of a negligible mass and initial abundance of the daughter particle, just to check
      // that we recover the decay DNCDM --> DR. This low initial abudance should be set here at the level of the PSD, not as a global factor in the momentum-integraded
      // quantities (in the past, I fixed this by absorbing the deg_factors inside the PSD, but that wasn't a good idea, leads to wrong results for scenarios B1/B2).
      // Hence, to set this low initial abundance one could just add here a small chemical potential, as in Eq. 4.6 of 2001.04466.
          pvecback_integration[index_bi_lnf_ncdm_decay_dr1_ + dncdm_properties.q_offset + index_q] = log(f0);
          pvecback_integration[index_bi_dlnfdlnq_separate_ncdm_decay_dr1_ + dncdm_properties.q_offset + index_q] = 0.;
        } else {
          double f0 = ncdm_->w_ncdm_[ncdm_id][index_q]/dncdm_properties.dq[index_q];
          double q = ncdm_->q_ncdm_[ncdm_id][index_q];
          pvecback_integration[index_bi_lnf_ncdm_decay_dr1_ + dncdm_properties.q_offset + index_q] = log(f0);
...
int BackgroundModule::background_output_data(int number_of_titles, double* data) const {
  ...
  for (index_tau = 0; index_tau < bt_size_; index_tau++){
    ...
    class_store_double(dataptr, pvecback[index_bg_a_], _TRUE_, storeidx);
    ...
    if (pba->has_ncdm == _TRUE_){
      for (n=0; n<pba->N_ncdm; n++){
        ...
        class_store_double(dataptr, pvecback[index_bg_rho_ncdm1_+n], _TRUE_, storeidx); //GFA
  ...
  // //         class_store_double(dataptr, pvecback[index_bg_dlnfdlnq_separate_ncdm_decay_dr1_ + ncdm_->decay_dr_map_[n].q_offset + i], _TRUE_, storeidx);
    ...
    if (pba->has_dr == _TRUE_) {
      for (int j = 0; j < pba->N_decay_dr; ++j) {
        class_store_double(dataptr, pow(pvecback[index_bg_a_],4)*pvecback[index_bg_rho_dr_species_ + j], _TRUE_, storeidx);
      }
    }
...
int BackgroundModule::background_derivs_member(
  ...
  double logf0_nu_dec; //GFA
  int idx_interp;
  double frac, threshold;
  ...
  if (pba->has_ncdm_decay_dr == _TRUE_){
    ...
    //  double g = 1.e-15;
    //  double T0_nu=0.71611;
    //  double mass_correction = (pow(ncdm_->M_ncdm_[0],2)-pow(ncdm_->M_ncdm_[1],2))*pow(ncdm_->M_ncdm_[0]+ncdm_->M_ncdm_[1],2)*pow(ncdm_->M_ncdm_[0],-4);
    //  double Gamma2 =  g*g*ncdm_->M_ncdm_[0]*mass_correction/(4.0*_PI_);
    //  Gamma2 *= (_k_B_*T0_nu*pba->T_cmb/((_h_P_/(2.*_PI_))*_c_))*_Mpc_over_m_;
    //  printf("Gamma2=%e km/s/Mpc \n",Gamma2*(_c_/1.e3));
    // NOTE: m_nu=0.1 eV and g=1e-15 (for normal hierarchy with degenerate decays), yields log10(Gamma/km/s/Mpc)=2.47
      ...
      for (int i = 0; i < ncdm_->q_size_ncdm_[ncdm_id]; i++) { // Loop over q grid points
        ...
        if (pba->has_ncdm_decay_dr_ncdm == _TRUE_) {
          if (dncdm_properties.dncdm_id == 0) {
            double deg_factor; // GFA
            if ((pba->is_ncdm_decay_degenerate == _TRUE_) && (pba->nu_hierarchy == normal)) {
              deg_factor = 2.;
            } else {
              deg_factor = 1.;
            }
            double decay_term = -a*a*M_ncdm*deg_factor*Gamma/epsilon;

            dy[index_bi_dlnfdlnq_separate_ncdm_decay_dr1_ + dncdm_properties.q_offset + i] = a*a*M_ncdm*deg_factor*Gamma*q*q/pow(epsilon, 3);
            dy[index_bi_lnf_ncdm_decay_dr1_ + dncdm_properties.q_offset + i] = decay_term;
          }

          if (dncdm_properties.dncdm_id == 1) {
            double M1 = ncdm_->M_ncdm_[ncdm_id - 1];
            double deg_factor; // GFA
            if ((pba->is_ncdm_decay_degenerate == _TRUE_) && (pba->nu_hierarchy == inverted)) {
              deg_factor = 2.;
            } else {
              deg_factor = 1.;
            }
            double pref = (a*a*deg_factor*Gamma*M1*M1*M1)/(epsilon*q*(M1*M1 - M_ncdm*M_ncdm));
            double f_q = exp(y[index_bi_lnf_ncdm_decay_dr1_ + dncdm_properties.q_offset + i]);
            double integral = 0.;
            const auto& prev_dncdm = ncdm_->decay_dr_map_[ncdm_id - 1];
            double q1p = abs((epsilon*(M1*M1 - M_ncdm*M_ncdm) + q*(M1*M1 + M_ncdm*M_ncdm))/(2*M_ncdm*M_ncdm));
            double q1m = abs((epsilon*(M1*M1 - M_ncdm*M_ncdm) - q*(M1*M1 + M_ncdm*M_ncdm))/(2*M_ncdm*M_ncdm));

           //GFA: Below I implemented the trick of performing the collision integral on a finer grid by interpolating the PSD
           // I also tried implementing it for the perturbed collision integral, but in that case the code becomes exceedingly slow
           if ((a>0.1*pba->a_NR) && (a<pba->a_decay2) && (pba->adjust_q_binning == _TRUE_) && (pba->nu_mass_gap_decay== solar)) {
              for (int j = 0; j < pba->Nq_interp; j++) {
                double dq = ncdm_->ncdm_qmax_[ncdm_id - 1]/pba->Nq_interp;
                double q1 = dq + j*dq;
                if (j == pba->Nq_interp - 1) {dq *= 0.5;}
                if ((q1 >= q1m) && (q1 <= q1p)) {
                  double epsilon1 = sqrt(q1*q1 + a*a*M1*M1);
                  // Below I perform cubic interpolation on array with equally space abscisses, following array_interpolate_cubic_equal in arrays.c
                  idx_interp = (int)floor((q1-ncdm_->q_ncdm_[ncdm_id - 1][0])/prev_dncdm.dq[10]);
                  if (idx_interp<1) idx_interp=1;
                  if (idx_interp>ncdm_->q_size_ncdm_[ncdm_id - 1]-3) idx_interp=ncdm_->q_size_ncdm_[ncdm_id - 1]-3;
                  frac = (q1-ncdm_->q_ncdm_[ncdm_id - 1][0])/prev_dncdm.dq[10]-idx_interp;
                  logf0_nu_dec = -y[index_bi_lnf_ncdm_decay_dr1_+prev_dncdm.q_offset+idx_interp-1]*frac*(1.-frac)*(2.-frac)/6.
                    +y[index_bi_lnf_ncdm_decay_dr1_+prev_dncdm.q_offset+idx_interp]*(1.+frac)*(1.-frac)*(2.-frac)/2.
                    +y[index_bi_lnf_ncdm_decay_dr1_+prev_dncdm.q_offset+idx_interp+1]*(1.+frac)*frac*(2.-frac)/2.
                    +y[index_bi_lnf_ncdm_decay_dr1_+prev_dncdm.q_offset+idx_interp+2]*(1.+frac)*frac*(frac-1.)/6.;
                  double w = dq*exp(logf0_nu_dec);
                  integral += w*q1/epsilon1;
                }
              }
            } else {
                for (int j = 0; j < ncdm_->q_size_ncdm_[ncdm_id - 1]; j++) {
                  double q1 = ncdm_->q_ncdm_[ncdm_id - 1][j];
                  if ((q1 >= q1m) && (q1 <= q1p)) {
                    double epsilon1 = sqrt(q1*q1 + a*a*M1*M1);
                    double dq = prev_dncdm.dq[j];
                    double w = dq*exp(y[index_bi_lnf_ncdm_decay_dr1_ + prev_dncdm.q_offset + j]);
                    integral += w*q1/epsilon1;
                }
              }
            }

            dy[index_bi_lnf_ncdm_decay_dr1_ + dncdm_properties.q_offset + i] = pref*integral/f_q;

            // GFA, we have to divide by f_q since we evolve lnf
            dy[index_bi_dlnfdlnq_separate_ncdm_decay_dr1_ + dncdm_properties.q_offset + i] = 0.;
            // GFA: I obtained the expression for the time derivative of dlnfdlnq of the daughter, but it's a bit messy to implement
            // In any case, we don't care so much about evolving this quantity, since the code also computes dlnfdlnq by doing the numerical derivative
            // However, to avoid memory problems, we should still write the corresponding derivative, that I set to zero for simplicity
          }
        ...
        for (const auto& [ncdm_id, dncdm_properties] : ncdm_->decay_dr_map_) {
            if (pba->has_ncdm_decay_dr_ncdm == _TRUE_) {
              if (dncdm_properties.dncdm_id == 0) {
                ...
                double Gamma = dncdm_properties.Gamma;
                double M_2 = ncdm_->M_ncdm_[ncdm_id + 1];
                double eps = 1./2.*(1.-(M_2*M_2)/(M_1*M_1));
                double deg_factor; // GFA
                if ((pba->is_ncdm_decay_degenerate == _TRUE_) && (pba->nu_hierarchy == normal)) {
                  deg_factor = 2.;
                } else {
                ...
                dy[index_bi_rho_dr_species_ + dncdm_properties.dr_id] = -4.*y[index_bi_a_]*pvecback[index_bg_H_]*y[index_bi_rho_dr_species_ + dncdm_properties.dr_id] + deg_factor*eps*y[index_bi_a_]*Gamma*M_1*pvecback[index_bg_number_ncdm1_ + ncdm_id]; //NT
              }
             } else {
              double M_1 = ncdm_->M_ncdm_[ncdm_id];
              double Gamma = dncdm_properties.Gamma;
